
package com.kara4k.mediagrub.model.flickr.userphotos;

import javax.annotation.Generated;
import com.google.gson.annotations.SerializedName;

@Generated("net.hexar.json2pojo")
@SuppressWarnings("unused")
public class Photo {

    @SerializedName("farm")
    private Long mFarm;
    @SerializedName("height_c")
    private Long mHeightC;
    @SerializedName("height_h")
    private Long mHeightH;
    @SerializedName("height_k")
    private Long mHeightK;
    @SerializedName("height_m")
    private String mHeightM;
    @SerializedName("height_n")
    private Long mHeightN;
    @SerializedName("height_o")
    private String mHeightO;
    @SerializedName("height_z")
    private String mHeightZ;
    @SerializedName("id")
    private String mId;
    @SerializedName("isfamily")
    private Long mIsfamily;
    @SerializedName("isfriend")
    private Long mIsfriend;
    @SerializedName("isprimary")
    private String mIsprimary;
    @SerializedName("ispublic")
    private Long mIspublic;
    @SerializedName("secret")
    private String mSecret;
    @SerializedName("server")
    private String mServer;
    @SerializedName("title")
    private String mTitle;
    @SerializedName("url_c")
    private String mUrlC;
    @SerializedName("url_h")
    private String mUrlH;
    @SerializedName("url_k")
    private String mUrlK;
    @SerializedName("url_m")
    private String mUrlM;
    @SerializedName("url_n")
    private String mUrlN;
    @SerializedName("url_o")
    private String mUrlO;
    @SerializedName("url_z")
    private String mUrlZ;
    @SerializedName("width_c")
    private String mWidthC;
    @SerializedName("width_h")
    private String mWidthH;
    @SerializedName("width_k")
    private String mWidthK;
    @SerializedName("width_m")
    private String mWidthM;
    @SerializedName("width_n")
    private String mWidthN;
    @SerializedName("width_o")
    private String mWidthO;
    @SerializedName("width_z")
    private String mWidthZ;

    public Long getFarm() {
        return mFarm;
    }

    public void setFarm(Long farm) {
        mFarm = farm;
    }

    public Long getHeightC() {
        return mHeightC;
    }

    public void setHeightC(Long heightC) {
        mHeightC = heightC;
    }

    public Long getHeightH() {
        return mHeightH;
    }

    public void setHeightH(Long heightH) {
        mHeightH = heightH;
    }

    public Long getHeightK() {
        return mHeightK;
    }

    public void setHeightK(Long heightK) {
        mHeightK = heightK;
    }

    public String getHeightM() {
        return mHeightM;
    }

    public void setHeightM(String heightM) {
        mHeightM = heightM;
    }

    public Long getHeightN() {
        return mHeightN;
    }

    public void setHeightN(Long heightN) {
        mHeightN = heightN;
    }

    public String getHeightO() {
        return mHeightO;
    }

    public void setHeightO(String heightO) {
        mHeightO = heightO;
    }

    public String getHeightZ() {
        return mHeightZ;
    }

    public void setHeightZ(String heightZ) {
        mHeightZ = heightZ;
    }

    public String getId() {
        return mId;
    }

    public void setId(String id) {
        mId = id;
    }

    public Long getIsfamily() {
        return mIsfamily;
    }

    public void setIsfamily(Long isfamily) {
        mIsfamily = isfamily;
    }

    public Long getIsfriend() {
        return mIsfriend;
    }

    public void setIsfriend(Long isfriend) {
        mIsfriend = isfriend;
    }

    public String getIsprimary() {
        return mIsprimary;
    }

    public void setIsprimary(String isprimary) {
        mIsprimary = isprimary;
    }

    public Long getIspublic() {
        return mIspublic;
    }

    public void setIspublic(Long ispublic) {
        mIspublic = ispublic;
    }

    public String getSecret() {
        return mSecret;
    }

    public void setSecret(String secret) {
        mSecret = secret;
    }

    public String getServer() {
        return mServer;
    }

    public void setServer(String server) {
        mServer = server;
    }

    public String getTitle() {
        return mTitle;
    }

    public void setTitle(String title) {
        mTitle = title;
    }

    public String getUrlC() {
        return mUrlC;
    }

    public void setUrlC(String urlC) {
        mUrlC = urlC;
    }

    public String getUrlH() {
        return mUrlH;
    }

    public void setUrlH(String urlH) {
        mUrlH = urlH;
    }

    public String getUrlK() {
        return mUrlK;
    }

    public void setUrlK(String urlK) {
        mUrlK = urlK;
    }

    public String getUrlM() {
        return mUrlM;
    }

    public void setUrlM(String urlM) {
        mUrlM = urlM;
    }

    public String getUrlN() {
        return mUrlN;
    }

    public void setUrlN(String urlN) {
        mUrlN = urlN;
    }

    public String getUrlO() {
        return mUrlO;
    }

    public void setUrlO(String urlO) {
        mUrlO = urlO;
    }

    public String getUrlZ() {
        return mUrlZ;
    }

    public void setUrlZ(String urlZ) {
        mUrlZ = urlZ;
    }

    public String getWidthC() {
        return mWidthC;
    }

    public void setWidthC(String widthC) {
        mWidthC = widthC;
    }

    public String getWidthH() {
        return mWidthH;
    }

    public void setWidthH(String widthH) {
        mWidthH = widthH;
    }

    public String getWidthK() {
        return mWidthK;
    }

    public void setWidthK(String widthK) {
        mWidthK = widthK;
    }

    public String getWidthM() {
        return mWidthM;
    }

    public void setWidthM(String widthM) {
        mWidthM = widthM;
    }

    public String getWidthN() {
        return mWidthN;
    }

    public void setWidthN(String widthN) {
        mWidthN = widthN;
    }

    public String getWidthO() {
        return mWidthO;
    }

    public void setWidthO(String widthO) {
        mWidthO = widthO;
    }

    public String getWidthZ() {
        return mWidthZ;
    }

    public void setWidthZ(String widthZ) {
        mWidthZ = widthZ;
    }

}
